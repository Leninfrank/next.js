...
describe('segment cache (staleness)', () => {
  ...
  it('expires dynamic prefetches when their stale time has elapsed', async () => {
    let page: Playwright.Page
    const browser = await next.browser('/', {
      beforePageLoad(p: Playwright.Page) {
        page = p
    ...
    // Reveal the links to trigger a dynamic prefetch
    const toggle5MinutesLink = await browser.elementByCss(
      'input[data-link-accordion="/dynamic-stale-5-minutes"]'
    )
    const toggle10MinutesLink = await browser.elementByCss(
      'input[data-link-accordion="/dynamic-stale-10-minutes"]'
    )
    await act(
      async () => {
        await toggle5MinutesLink.click()
    ...
    await act(
      async () => {
        await toggle10MinutesLink.click()
        await browser.elementByCss('a[href="/dynamic-stale-10-minutes"]')
      },
      {
        includes: 'Content with stale time of 10 minutes',
      }
