...
describe('<Link prefetch={true}> (dynamic)', () => {
  const { next, isNextDev, isNextDeploy } = nextTestSetup({
    files: __dirname,
    // TODO: enable deploy tests when support for `next-router-prefetch: 2` is merged
    // https://github.com/vercel/vercel/pull/13547
    skipDeployment: true,
  })
  if (isNextDev) {
  ...
  describe('errors', () => {
    it('aborts the prerender and logs an error when sync IO is used after cookies()', async () => {
      // In a dynamic prefetch, we might encounter sync IO usages that weren't caught during build,
      ...
      await act(async () => {
        const linkToggle = await browser.elementByCss(
          `input[data-prefetch="dynamic"][data-link-accordion="/errors/sync-io-after-cookies"]`
        )
        await linkToggle.click()
      }, [
        // Should include the shell
      ...
      // TODO(dynamic-ppr): we should be able to display the (aborted, partial) prefetched contents before navigating,
    ...
    it('should trigger error boundaries for errors that occurred in dynamically prefetched content', async () => {
      ...
      await act(async () => {
        const linkToggle = await browser.elementByCss(
          `input[data-prefetch="dynamic"][data-link-accordion="/errors/error-after-cookies"]`
        )
        await linkToggle.click()
      }, [
        // Should include the shell
