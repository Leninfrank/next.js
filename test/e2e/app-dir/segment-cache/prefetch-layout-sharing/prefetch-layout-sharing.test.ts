...
describe('layout sharing in non-static prefetches', () => {
  ...
  // - A "dynamic prefetch" is the new `<Link prefetch={true}>` (only available in cacheComponents mode).
  //   It includes cached IO, and allows access to cookies/params/searchParams/"use cache: private", but excludes uncached IO.

  it('dynamic prefetches should omit layouts that were already prefetched with a dynamic prefetch', async () => {
    ...
    // Reveal the link to trigger a dynamic prefetch for page one
    await act(async () => {
      const linkToggle = await browser.elementByCss(
        `input[data-prefetch="dynamic"][data-link-accordion="/shared-layout/one"]`
      )
    ...
    // Reveal the link to trigger a dynamic prefetch for page two
    await act(async () => {
      const linkToggle = await browser.elementByCss(
        `input[data-prefetch="dynamic"][data-link-accordion="/shared-layout/two"]`
      )
      await linkToggle.click()
    }, [
  ...
  it('dynamic prefetches should omit layouts that were already prefetched with a full prefetch', async () => {
  ...
  it('full prefetches should include layouts that were only prefetched with a dynamic prefetch', async () => {
  ...
  it('full prefetches should omit layouts that were prefetched with a dynamic prefetch and had no dynamic holes', async () => {
    // If a dynamic prefetch gave us a complete segment with no dynamic holes left, then it's equivalent to a full prefetch.
    //
    // TODO: This doesn't work in all cases -- if any segment in a dynamic prefetch was partial, we'll mark all of them as partial,
    ...
    await act(async () => {
      const linkToggle = await browser.elementByCss(
        `input[data-prefetch="dynamic"][data-link-accordion="/dynamically-prefetchable-layout/one"]`
      )
      await linkToggle.click()
    }, [
      // should prefetch page one, and allow reading cookies
    ...
    // Navigate to page one. It should have been completely prefetched by the dynamic prefetch.
    await act(async () => {
      await browser
        .elementByCss(`a[href="/dynamically-prefetchable-layout/one"]`)
        .click()
    }, 'no-requests')
    ...
    await act(async () => {
      const linkToggle = await browser.elementByCss(
        `input[data-prefetch="full"][data-link-accordion="/dynamically-prefetchable-layout/two"]`
      )
      await linkToggle.click()
    }, [
    ...
    await act(async () => {
      await browser
        .elementByCss(`a[href="/dynamically-prefetchable-layout/two"]`)
        .click()
    }, 'no-requests')
  ...
  it('navigations should omit layouts that were prefetched with a dynamic prefetch and had no dynamic holes', async () => {
    // If a dynamic prefetch gave us a complete segment with no dynamic holes left, then it's equivalent to a full prefetch.
    //
    // TODO: This doesn't work in all cases -- if any segment in a dynamic prefetch was partial, we'll mark all of them as partial,
    // which means they can't be reused in a full prefetch or a navigation. So this only works if the dynaimic prefetch has no holes at all.
    ...
    await act(async () => {
      const linkToggle = await browser.elementByCss(
        `input[data-prefetch="auto"][data-link-accordion="/dynamically-prefetchable-layout/two"]`
      )
      await linkToggle.click()
    })
